...
begin
  ...
  begin
    ...
    if v_cancel_pos is not null then
      select coalesce(min(queue_position), 0) - 1000000
        into v_shift
      from public.registrations
      where game_id = _game_id
      ...
      with shifted as (
        update public.registrations
        set queue_position = queue_position + v_shift
        where game_id = _game_id
          and status in ('active', 'standby')
      ...
      set queue_position = r.queue_position - v_shift - 1,
          updated_at = now()
      where r.id in (select id from shifted);
    end if;
        ...
        if v_cancel_pos is not null then
          ...
          with shifted as (
            ...
            where r.game_id = _game_id
              ...
              and r.queue_position > v_cancel_pos
            returning r.id
          )
          update public.registrations r
          set queue_position = r.queue_position - v_shift - 1,
