alter table if exists public.registrations
  ...
  add column if not exists is_early_finish boolean not null default false,
  add column if not exists early_finish_time time;
...
begin
  ...
  if v_existing.status not in ('active', 'standby') then
    raise exception 'Registration not active';
  end if;

  if v_existing.check_in_status = 'checked_in' then
    if _is_late is distinct from v_existing.is_late
       or _eta_minutes is distinct from v_existing.eta_minutes then
      raise exception 'Cannot update late after check-in';
    end if;
  end if;

  if _is_late then
    if _eta_minutes is not null and _eta_minutes <= 0 then
      raise exception 'eta_minutes must be positive';
    end if;
    v_eta_minutes := _eta_minutes;
  else
    v_eta_minutes := null;
  end if;

  if _is_early_finish then
    v_early_finish_time := _early_finish_time;
  else
    v_early_finish_time := null;
  end if;

  update public.registrations r
  set is_late = _is_late,
      eta_minutes = v_eta_minutes,
      is_early_finish = _is_early_finish,
      early_finish_time = v_early_finish_time,
      updated_at = now()
