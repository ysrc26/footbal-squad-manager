...
begin
  ...
  begin
    ...
    if _action = 'checkin_on' then
      ...
      set r.check_in_status = 'checked_in',
          r.updated_at = now()
      where r.id = v_registration.id;
    elsif _action = 'checkin_off' then
      update public.registrations r
      set r.check_in_status = 'pending',
          r.updated_at = now()
      where r.id = v_registration.id;
      ...
      if v_registration.status <> 'cancelled' then
        ...
        if v_cancel_pos is not null then
          update public.registrations r
          set r.queue_position = r.queue_position - 1,
              r.updated_at = now()
          where r.game_id = _game_id
