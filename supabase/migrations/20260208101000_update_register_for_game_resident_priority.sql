...
begin
  ...
  begin
    ...
    if v_wave2 is not null and v_now < v_wave2 then
      with ordered as (
        select
          r.id,
          row_number() over (
            order by coalesce(p.is_resident, false) desc,
      ...
      update public.registrations r
      set status = case when r.queue_position <= v_max_players then 'active' else 'standby' end,
          updated_at = now()
      where r.game_id = _game_id
        and r.status in ('active', 'standby');
    end if;
